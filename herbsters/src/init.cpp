 ...
 * between herbstersd, and herbsters-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("herbsters-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), herbsters_CONF_FILENAME));
    if (mode == HMM_herbstersD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), herbsters_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/herbsters-project/herbsters>";
    const std::string URL_WEBSITE = "<https://herbsters.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("herbsters-loadblk");
...
/** Sanity checks
 *  Ensure that herbsters is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single herbsters process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", herbsters_CONF_FILENAME)).string());
